...
# Train SVM model
model = MultiOutputClassifier(LinearSVC(class_weight="balanced", max_iter=5000))
...
for train_idx, val_idx in kf.split(X):
    ...
    fold_model = MultiOutputClassifier(LinearSVC(class_weight="balanced", max_iter=5000))
...
print(f"\nSVM Avg Cross-Validated Accuracy: {np.mean(accuracies):.3f} ± {np.std(accuracies):.3f}")
print(f"SVM Avg Cross-Validated Macro F1: {np.mean(f1_scores):.3f} ± {np.std(f1_scores):.3f}")

# Save trained model and vectorizer
#joblib.dump(model, 'SVM/svm_sentiment_multilabel_model.joblib')
#joblib.dump(vectorizer, 'SVM/tfidf_vectorizer_sentiment.joblib')

joblib.dump(model, r"models\SVM_topic\svm_sentiment_multilabel_model.joblib")
joblib.dump(vectorizer, r"models\SVM_topic\svm_tfidf_vectorizer_sentiment.joblib")

print("\nModel and vectorizer saved successfully:")
print("- Model file: models/svm_sentiment_multilabel_model.joblib")
print("- Vectorizer file: models/tfidf_vectorizer_sentiment.joblib")
